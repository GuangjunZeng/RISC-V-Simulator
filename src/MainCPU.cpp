...
bool isDisableDataForwarding = false;
...
int main(int argc, char **argv) {
    ...
    simulator.isDisableDataForwarding = isDisableDataForwarding;
...
void printUsage() {
    std::cout << "Usage: Simulator riscv-elf-file [-v] [-s] [-d] [-b param] [-x] [-h]\n";
    std::cout << "Parameters: \n\t[-v] verbose output \n\t[-s] single step\n";
    std::cout << "\t[-d] dump memory and register trace to dump.txt\n";
    std::cout << "\t[-b param] branch perdiction strategy, accepted param AT, NT, "
                 "BTFNT, BPB\n";
    std::cout << "\t[-x] disable data forwarding" << std::endl;
...
void printElfInfo(ELFIO::elfio *reader) {
    ...
    for (int i = 0; i < sec_num; ++i) {
        ...
        printf("[%d]\t%-12s\t0x%lx\t%ld\n", i, psec->get_name().c_str(), psec->get_address(), psec->get_size());
    }
    ...
    for (int i = 0; i < seg_num; ++i) {
        ...
        printf("[%d]\t0x%x\t0x%lx\t%ld\t%ld\n", i, pseg->get_flags(), pseg->get_virtual_address(),
